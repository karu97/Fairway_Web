  ...
  return (
    <MainLayout>
      <section className="mx-auto max-w-7xl px-4 py-12">
        <h1 className="text-3xl font-serif">Our Hotels</h1>
        <p className="text-black/70 mt-2">Stay at design-forward spaces in Sri Lanka’s most captivating locations.</p>
          ...
          {hotels.map((h) => (
            <article key={h.id} className="rounded-xl overflow-hidden border border-black/10 hover:shadow-sm transition bg-white">
              ...
              <div className="p-4">
                ...
                <p className="text-sm text-black/60">{h.location} • {h.perk}</p>
                {h.slug ? (
                  <Link href={`/hotels/${h.slug}`} className="inline-block mt-3 px-4 py-2 text-sm rounded-md border border-black/15 hover:bg-black/5">View Details</Link>
                ) : (
                  <span className="inline-block mt-3 px-4 py-2 text-sm rounded-md border border-black/15 text-black/50">Coming Soon</span>
                )}
              </div>
            </article>
          ))}
